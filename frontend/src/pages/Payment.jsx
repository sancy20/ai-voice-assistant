...
const TOKEN_PACKS = {
  50:   { label: "Starter Token Pack",  price: 0.99, tokens: 50 },
  200:  { label: "Plus Token Pack",     price: 2.99, tokens: 200 },
  500:  { label: "Power Token Pack",    price: 5.99, tokens: 500 },
  1000: { label: "Max Token Pack",      price: 9.99, tokens: 1000 },
};
...
function CardPreview({ num, expiry, holder, cvv, flipped }) {
  ...
  return (
    <div style={{ perspective: "900px", height: "clamp(120px, 28vw, 155px)", marginBottom: 4 }}>
      <motion.div
...
export default function Payment() {
  ...
  const purchaseType = params.get("type") ?? "plan";
  const planKey  = params.get("plan") ?? "pro";
  const tokenAmount = Number(params.get("tokens") ?? 0);
  const plan     = PLAN_DATA[planKey] ?? PLAN_DATA.pro;
  const tokenPack = TOKEN_PACKS[tokenAmount] ?? TOKEN_PACKS[200];
  const item = purchaseType === "tokens" ? tokenPack : plan;
  const tax      = +(item.price * TAX_RATE).toFixed(2);
  const total    = +(item.price + tax).toFixed(2);
  const checkoutPayload = purchaseType === "tokens" ? { purchase_type: "tokens", tokens: item.tokens, } : { purchase_type: "plan", plan: planKey, };
  ...
  const orderSummary = (
    <div className="space-y-3">
      <div className="rounded-xl border overflow-hidden" style={{ background: "var(--surface)", borderColor: "var(--border)", marginTop: "15px"}}>
        <div className="px-4 py-3 border-b" style={{ borderColor: "var(--border-s)" }}>
          <p className="text-xs font-semibold uppercase tracking-widest" style={{ color: "var(--fg-4)" }}> Order summary</p>
        </div>
        <div className="p-4 space-y-3">
          <div className="flex items-start justify-between">
            <div> <p className="text-sm font-semibold" style={{ color: "var(--fg)" }}> {item.label}</p>
              <p className="text-xs mt-0.5" style={{ color: "var(--fg-4)" }}> {purchaseType === "tokens" ? "One-time token purchase" : `Billed ${plan.period}`}</p>
            </div>
            <p className="text-sm font-medium" style={{ color: "var(--fg)" }}>${item.price.toFixed(2)}</p>
          </div>
          <div className="flex justify-between text-xs pt-1" style={{ color: "var(--fg-4)" }}><span>Tax (9%)</span><span>${tax.toFixed(2)}</span></div>
          <div className="flex justify-between items-center pt-3 border-t" style={{ borderColor: "var(--border-s)" }}>
            <span className="text-sm font-semibold" style={{ color: "var(--fg)" }}>Total due</span>
            <span className="text-lg font-bold" style={{ color: "var(--fg)" }}>${total.toFixed(2)}</span>
          </div>
        </div>
      </div>
      <div className="rounded-xl border p-4 space-y-2.5"style={{ background: "var(--surface)", borderColor: "var(--border)", marginTop: "15px"}}>
        <p className="text-[11px] font-semibold uppercase tracking-widest mb-3" style={{ color: "var(--fg-4)" }}>Included</p>
        {(purchaseType === "tokens"
            ? [
                `${item.tokens} extra AI tokens added to your balance`,
                "One-time token purchase only",
      ...
      <div className="flex items-center justify-center gap-2 py-1" style={{ color: "var(--fg-4)" }}>
  ...
  const handlePay = async (e) => {
    ...
    try {
      const res = await fetch(`${API_URL}/subscription/mock-payment/confirm`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          card_last4: cardNum.replace(/\s/g, "").slice(-4) || null,
        }),
      });
      ...
      <div className="min-h-full flex flex-col items-center justify-center px-4 pt-32 pb-16 lg:pt-40">
        <motion.div
          ...
          <div className="rounded-xl border divide-y overflow-hidden text-left"
            ...
            {[
              [purchaseType === "tokens" ? "Token Pack" : "Plan", item.label],
              [ "Billing", purchaseType === "tokens" ? "One-time purchase" : plan.period.replace("/", "").trim() ],
  ...
  /* ── Main payment UI ── */
  return (
    <div className="w-full px-4 sm:px-6 lg:px-8 py-5 sm:py-8 pb-4 lg:pb-8 flex justify-center">
      <div className="w-full max-w-[920px]">

        {/* Header */}
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.32, ease: [0.16, 1, 0.3, 1] }}
          className="flex items-center gap-3 mb-5 sm:mb-6" style={{marginBottom: "10px"}}>
          <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,580px)_300px] gap-5 lg:gap-6 items-start justify-center">
            ...
            <AnimatePresence mode="wait">
              ...
              {method === "card" && (
                <motion.div key="card"
                  ...
                  className="space-y-4 w-full" style={{marginTop: "15px"}}>

                  <CardPreview num={cardNum} expiry={expiry} holder={holder} cvv={cvv} flipped={flipped} />

                  <form onSubmit={handlePay} className="space-y-4 w-full" style={{marginTop: "15px"}}>
          ...
          <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }}
            ...
            className="hidden lg:block space-y-3 h-fit w-full max-w-[360px] lg:mx-0 lg:sticky lg:top-8">
            {orderSummary}
          </motion.div>
        </div>
      </div>
